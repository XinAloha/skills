...
```text
You have access to a local skill named overseas-equity-factor-miner at:
<OVERSEAS_EQUITY_FACTOR_MINER_SKILL_ROOT>

When the user request matches this skill's SKILL.md description:
1. Read <OVERSEAS_EQUITY_FACTOR_MINER_SKILL_ROOT>/SKILL.md.
2. Follow the workflow and guardrails in that file exactly.
3. Load referenced files under <OVERSEAS_EQUITY_FACTOR_MINER_SKILL_ROOT>/references/ only when needed.
