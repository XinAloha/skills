...
> 后续这里会继续接入社区其它 skill 项目；当前已包含 **107 个 skill**：54 个从外部仓库导入 + 53 个本仓库原生。完整清单见 [`INDEX.md`](INDEX.md)。

## ⭐ 置顶推荐 — `workmode/manage-upward` 向上管理模式

**[workmode/manage-upward.md](workmode/manage-upward.md)** 是本仓库目前最优先推荐的协作类 skill，用于让"执行者 ↔ 合作者"互相督促，把 AI 和用户都拉到合格打工人的状态。

- **核心理念**：向上管理 ≠ 拍马屁，本质是**用你的可控性，去覆盖对方的不确定感和焦虑感**。
- **三件事汇报法**：每次同步只讲清 *进度 / 风险 / 需要的支持* 这三件事，把对方焦虑感降到最低。
- **2 小时风险上报原则**：任何不能把握的风险，**自己最多硬扛 2 小时**，超时立刻带"已尝试 + 临时结论"上报，杜绝闷头干 → 工期失控。
- **汇报问题必带方案**：哪怕方案不成熟，也比当传声筒强。展现主动性，减轻对方决策负担。
- **摆脱学生思维**：你是打工人不是来交钱学习的，**少看多干、工作留痕、及时沟通**。多用"我们"代替"我"和"你"。
- **本项目里是双向的**：AI 会主动按这些原则提醒用户，用户也用同样原则要求 AI（汇报、上报风险、带方案）。

启用方式：用户在对话里说 *"向上管理 / 打工人模式 / 督促我"* 或调用 `/manage-upward`，AI 即按此 skill 协作。详见 [`workmode/manage-upward.md`](workmode/manage-upward.md)。
