...
6. 将实施者与验证者分离。源码写入任务必须调用 `$verify-loop-delivery`；验证者仅根据契约、差异、命令输出和运行时证据判定，未通过则把反馈写回任务状态。需要模型或目标驱动终止时读取 [references/reviewer-and-goal.md](references/reviewer-and-goal.md)。
